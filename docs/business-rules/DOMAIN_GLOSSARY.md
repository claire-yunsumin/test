...
Task는 현재 Template만 해석하지 않고 생성/적용 시점의 Template/Form/Workflow/Approval Policy snapshot을 함께 보관합니다. 현재 데모 구현은 embedded snapshot이며, 운영 DB 전환 목표는 immutable VersionRef입니다.
...
`PENDING_APPROVAL`은 상태 매핑용 category입니다. 실제 승인 대기 여부는 문자열 포함 여부가 아니라 열린 `ApprovalRequest(status=PENDING)` 존재 여부로 판단합니다.
...
핵심 이벤트는 `TASK_CREATED`, `TASK_UPDATED`, `TASK_TRANSITIONED`, `APPROVAL_REQUESTED`, `APPROVAL_APPROVED`, `APPROVAL_REJECTED`, `APPROVAL_SUPPLEMENT_REQUESTED`, `TEMPLATE_SNAPSHOT_APPLIED`, `NOTE_REFERENCED` 등을 포함합니다.

### Inbox Item

사용자에게 다음 행동 신호를 전달하는 알림성 항목입니다. `readAt`, `ackAt`, `remindCount`를 가집니다.

화면에서는 `userId === me`인 항목을 수신함에 표시하고, `sourceUserId === me`이면서 다른 수신자에게 향한 항목을 발신함 추적 패널에 표시합니다.
...
Task의 특정 상태 전이에 대해 열린 승인 런타임입니다. `fromStatusId`, `targetStatusId`, 요청자, 요청 사유, `policySnapshot`, `status`를 보관합니다.

### Approval Decision

Approval Request에 대한 승인자의 판단 기록입니다. `APPROVE`, `REJECT`, `SUPPLEMENT_REQUEST`와 판단 사유, 판단 시각을 보관합니다.
...
`layout/Shell`이 제공하는 좌측 영역입니다. GNB(홈·알림함·태스크·설정 등)와 태스크 화면에서만 펼쳐지는 **워크스페이스 Explorer**로 구성되며, Explorer는 Unit → Folder → List 트리를 보여 줍니다. 팀즈의 팀/채널 화면은 정보 구조를 잡기 위한 **참고 IA**이고, 구현과 용어는 본 문서의 Unit/Folder/List를 따릅니다.

### Home Dashboard

`/home` 첫 진입 화면입니다. 전체 `/tasks` 목록 대신 결정 대기, 내 활성 태스크, 오늘/임박 항목, 참관 업데이트를 먼저 보여 줍니다. 이전에 태스크 작업대에서 선택한 Unit/List 범위와 무관하게 현재 사용자에게 보이는 전체 유닛 정보를 사용합니다.
...
태스크 상세 우측 영역입니다. 사용자에게는 `논의`와 `변경 기록` 탭으로 표시됩니다. 코드와 데이터 모델에서는 각각 thread/comment, timeline event를 사용합니다.
