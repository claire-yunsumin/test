...
Workspace 맥락은 `Unit -> Folder -> List -> Task`로 이어지며, 태스크 생성/수정 시 `unitId`, `folderId`, `listId` 조합 무결성을 서버가 강제합니다.
...
- 태스크 리스트: unit, folder, list 필터와 메타 정보를 표시합니다.
- 보드/백로그: list와 phase 맥락을 함께 사용합니다.
...
- `packages/shared/src/index.ts`: `Unit`, `UnitMember`, `Folder`, `TaskList`, `Task`
