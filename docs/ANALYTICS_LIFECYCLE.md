...
- `voluntaryVisitCount`
- `alarmActionConversionRate`
- `decisionClosureRate`
- `templateStatusMappingSuccessRate`
- `templateManualAdjustmentRate`
- `computedAt`
- `dataStatus`
...
- `timeline`: 결정 이벤트 수, 승인 요청 대비 종결률, 템플릿 상태 매핑 성공률
- `engagement`: 재방문, 수정, Template 적용, Form 저장, 피드백 후 수정률

O1 자동 판정은 단순 방문 수가 아니라 `Alarm->Action`, 결정 귀속 완결률, 주간 자발 재방문율, 템플릿 매핑 성공률을 함께 봅니다. `computedAt`과 `dataStatus`는 현재 지표가 최신 계산값인지 fallback 값인지 화면에서 구분하기 위한 메타데이터입니다.
